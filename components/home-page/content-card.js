...
function ContentCard({ item, t }) {
  const getTypeLabel = () => {
    switch (item.type) {
      case 'project': return t('projectType');
      case 'tutorial': return t('tutorialType');
  ...
  return (
    <article className={classes.card}>
      ...
      <div className={classes.statusBadges}>
        {item.featured && (
          <span className={`${classes.statusBadge} ${classes.featured}`}>
            {t('featuredLabel')}
          </span>
        )}
        {item.trending && (
          <span className={`${classes.statusBadge} ${classes.trending}`}>
            {t('trendingLabel')}
          </span>
        )}
        {item.new && (
      ...
      <div className={classes.content}>
        ...
        <div className={classes.footer}>
          {item.stats ? (
            <div className={classes.stats}>
              <span className={classes.stat}>
                ...
                <span className={classes.statValue}>{item.stats.stars}</span>
                <span className={classes.statLabel}>{t('stars')}</span>
              </span>
              <span className={classes.stat}>
                <span className={classes.statIcon}>🔀</span>
                <span className={classes.statValue}>{item.stats.forks}</span>
                <span className={classes.statLabel}>{t('forks')}</span>
              </span>
            </div>
          ) : (
          ...
          <div className={classes.actions}>
            ...
            >
              {item.type === 'article' ? t('readArticle') : t('viewCode')}
            </Link>
          </div>
        </div>
      </div>
