...
// Новая функция: получаем только базовые имена файлов (без языковых расширений)
export function getUniquePostsFiles() {
    const allFiles = fs.readdirSync(postsDirectory);

    // Извлекаем уникальные имена (удаляем .ru, .en, .md)
    const uniqueSlugs = new Set();

    allFiles.forEach((file) => {
        // Оставляем только базовое имя без расширений языка
        const baseName = file
            .replace(/\.(en|ru)\.md$/, "") // удаляем .en.md или .ru.md
            .replace(/\.md$/, ""); // удаляем просто .md
...
export function getPostData(postIdentifier, locale = "en") {
    ...
    // Если нет файла с языком, пробуем базовый файл
...
// Новая функция: получаем только уникальные посты (без переводов)
export function getUniquePosts(locale = "en") {
    ...
    const uniquePosts = uniqueSlugs
        ...
        .filter((post) => {
            // Фильтруем: если это перевод (не английская версия),
            // но есть английская версия - не включаем в список
